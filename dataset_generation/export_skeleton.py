...
OUTPUT_PATH = "/data_hdd/datasets/wire_harness_tim/data/labels"
...
"""
Get the bone length using the assigned poitns of
"""
...
def segment_from_bone(R_a, R_o, armature, ob, start_bone_id, is_end, group_lookup, labels, connector_bones):
    ...
    # print("start_overhead: {}".format(start_overhead))
...
def extract_skeleton(R_a, R_o, armature, ob, current_bone, bone_id, ):


def child_bones_wo_connectors(bone, connector_bones):
    return [c for c in bone.children if c.name not in connector_bones]

def get_skeleton(R_a, R_o, armature, ob, start_bone_id, group_lookup, connector_bones, is_start=True):
...
def export_labels(R, obj, labels, reserved_counts, output_file):
    ...
    # labeled = np.c_[
    #     np.r_[coords, connector_coords, outlier_coords],
    #     np.r_[labels, np.full(len(connector_coords), 3), np.full(len(outlier_coords), 4)]
    # ]
    # print("Saving")
    ...
    # np.savetxt(output_file, labeled, delimiter=",")
...
if __name__ == "__main__":
    ...
    segments_path = output_path / "set2" / sample / "segments"
    ...
    for frame in range(1, 301):
        ...
        # if we need the connector bones as well
        # for o in connector_meshes:
        #     group_lookup = build_group_lookup(o) | group_lookup
        #     print(o.name)

        segments, segment_lens = get_segment_lengths(R_a, R_o, armature, main_ob, bone_id, group_lookup, connector_bones)
        
        labeled_segments = [np.c_[seg, np.full(len(seg), i)] for i, seg in enumerate(segments)]

        labeled_pcl = np.concatenate(labeled_segments)

        np.save(segments_path / "{:03d}.npy".format(frame - 1), labeled_pcl)
        # np.save(output_path / "set2" / sample / "segment_lengths.npy", segment_lens)

        # print("saving to ", segments_path / "{:03d}.npy".format(frame - 1))
        ...
        # export_labels(R_o, main_ob, labels, reserved_counts, output_path / "set2" / sample / "segmentation.npy")
